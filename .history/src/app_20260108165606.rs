...
impl eframe::App for VideoPlayerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("side_panel").min_width(200.0).show(ctx, |ui| {
            ...
            // Favorites
            ui.checkbox(&mut self.show_favorites_only, &self.i18n.t("favorites_only"));
            ui.separator();
            ...
            let fav_text = self.i18n.t("favorites_count").replace("{}", &self.database.get_favorites().len().to_string());
            ui.label(&total_text);
            ui.label(&fav_text);
        });
            ...
            egui::SidePanel::right("scene_panel").exact_width(300.0).show(ctx, |ui| {
                if let Some(video_id) = &self.selected_video.clone() {
                    ...
                    if let Some(video) = video_data {
                        ...
                        // Favorite button
                        let fav_button_text = if video.is_favorite {
                            self.i18n.t("remove_from_favorites")
                        } else {
                            self.i18n.t("add_to_favorites")
                        };
                        
                        if ui.button(egui::RichText::new(&fav_button_text).size(16.0)).clicked() {
                            self.toggle_favorite(video_id);
                        }
                        ...
                        if let Some(ref thumb_path) = video.thumbnail_path {
                            ...
                            if ui.is_rect_visible(rect) {
                                if let Some(texture) = self.load_image_texture(ctx, thumb_path) {
                                    ...
                                    // Draw favorite highlight (yellow border)
                                    if video.is_favorite {
                                        painter.rect_stroke(
                                            rect,
                                            4.0,
                                            egui::Stroke::new(4.0, egui::Color32::from_rgb(255, 215, 0))
                                        );
                                    }
