...
pub fn get_available_shaders() -> Vec<String> {
    ...
    // Check multiple possible locations for glsl_shaders directory
    let shader_paths = [
        "mpv\\glsl_shaders",
        ".\\mpv\\glsl_shaders",
        "..\\mpv\\glsl_shaders",
    ...
    for shader_dir in &shader_paths {
        let path = std::path::Path::new(shader_dir);
...
/// Get GLSL shader file paths for enabled shaders (relative to mpv.exe)
fn get_shader_files(selected_shader: Option<&str>) -> Vec<String> {
    ...
    if let Some(shader_name) = selected_shader {
        ...
        for shader_dir in &shader_dirs {
            let dir_path = std::path::Path::new(shader_dir);
            
            if dir_path.exists() && dir_path.is_dir() {
                let shader_path = dir_path.join(shader_name);
                
                if shader_path.exists() {
                    // Convert to path relative to mpv.exe (which is in mpv/ directory)
                    // So mpv\glsl_shaders\shader.glsl becomes glsl_shaders\shader.glsl
                    let relative_path = format!("glsl_shaders\\{}", shader_name);
                    shader_paths.push(relative_path);
                    break;
                }
            }
        }
...
pub fn play_video_at_timestamp(video_path: &Path, timestamp_seconds: f64, always_on_top: bool, use_gpu_hq: bool, use_custom_shaders: bool, selected_shader: Option<&str>, use_frame_interpolation: bool, volume: u8) -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "windows")]
    {
        // Try local mpv.exe first (best IPC support)
        let local_mpv_paths = [
            "mpv\\mpv.exe",
            ".\\mpv\\mpv.exe",
            "..\\mpv\\mpv.exe",
        ...
        for mpv_path in &local_mpv_paths {
            let path = std::path::Path::new(mpv_path);
            ...
            if path.exists() {
                ...
                eprintln!("🚀 Launching mpv: {} {:?}", mpv_path, args);
                
                // Convert to &str refs
                let args_refs: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
                
                if try_play_video(mpv_path, &args_refs) {
                    return Ok(());
                }
            }
        }
